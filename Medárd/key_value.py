...
with open("Medárd/data.json", "r") as f:
    data = json.load(f)


users_list = []
for i in range(10000):
    user = {"name": data["names"][i], "email": data["emails"][i], "age": data["ages"][i]}
    users_list.append(user)


uri = "mongodb+srv://mergesenektanar69:lt2UeiRMXToEgYpH@cluster0.t9b0xbs.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
database_name = "User_database_"
...
def write_to_mongodb(users_list):
    times = []
    delete_times = []
    for _ in range(10):
        ...
        start = time.time()
        db[collection_name].insert_many(users_list)
        end = time.time()
...
def plots(time_list_write, time_list_delete, time_list_query):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("Medárd", "imgs", "key_value.png"))
    plt.show()


time_list_write, time_list_delete = write_to_mongodb(users_list)
