...
def write_to_neo4j(users_list):
    times = []
    database_connection = GraphDatabase.driver(uri="bolt://localhost:7687", auth=("neo4j", "comparegraphs"))
...
def do_query():
    ...
    times = []
    query = """
    MATCH (u:User)-[:EMAIL]->(e:Email), (u)-[:AGE]->(a:Age)
    RETURN u.name AS Name, e.address AS Email, a.value AS Age
    """
    time_list = []
    for _ in range(10):
        start = time.time()
    ...
    for record in result:
        data.append((record["Name"], record["Email"], record["Age"]))
